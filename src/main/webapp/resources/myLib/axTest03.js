...
/*
 * Test 2. 타이틀 클릭하면, 글목록의 아랫쪽(<tr>추가 : span)에 글 내용 출력하기 => Toggle 방식으로 없을때
 * 클릭하면 표시되고, 있을때 클릭하면 사라지도록
 */
...
/*
 * Test 3. seq 에 마우스 오버시에 별도의 DIV에 글내용 표시 => jQuery : id, class, this =>
 * hover(f1,f2); => 3.1) css 활용 : display 속성
 */
	...
	$('.sss1').hover(function(e) {
		...
		$.ajax({
			...
			success : function(resultData) {
				...
				$('.content').html('');
				$('#content').html(resultData.content).css({
					display: "block",
					left: mleft,
					top: mtop
				});//css
