...
$(function() {
	...
	// 그러므로 Java의 Data를 JS가 인식가능한 Data 형식(JSON 포멧)으로 변환해야함.
	...
	$('#jslogin').click(function() {
		$.ajax({
			...
			success : function(resultData) {
				// => 서버로부터 Json 형태의 Data를 Response로 받음
				// "loginSuccess" -> "T" 성공 , 새로고침
				// "loginSuccess" -> "F" 실패
				// => 성공시 : 성공 message, 새로고침
				// 실패시 : 실패 message, 재로그인 유도
...
//		
		...
		// => contentType:false, // false로 선언시 content-type 헤더가
		// multipart/form-data로 전송되게 함
		...
		$.ajax({
			...
			error : function() {
				$('#resultArea1').html("ajax_axjoin_test error");
			}
		});
	});// axjoin
	/*
